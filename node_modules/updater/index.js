...
class Updater {
	...
	async updateSelf(dir, manifest, files, deleted) {
		...
		let script = ':start'
		script += `\nstart "Waiting for application to exit" /w /min "${path.resolve(path.join(dir, 'bin/node/node.exe'))}" "${path.resolve(path.join(dir, 'bin/wait-for-exit'))}"`
